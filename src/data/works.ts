...
export const works: readonly Work[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      chapter('result', '結果・到達点', '5つの頭脳を同じ場所で戦わせる。', ['評価関数や探索方式を差し替えて対戦でき、手法ごとの壁の使い方と経路選択の差を観察できました。']),
  ...
  },
  {
    ...
    sections: [
      ...
      chapter('result', '結果・到達点', '最大100万式を、意味のある場所へ配分。', ['輪郭497本、セグメント41,385個、塗り潰し領域815個を一つの変換パイプラインで扱えるようにしました。']),
  ...
  },
  {
    ...
    sections: [
      ...
      chapter('result', '結果・到達点', '42KBの選択履歴から、生成の傾向が変化。', ['タコのような線画から抽象的な形まで、ノイズ8段階を比較できる実験系を構築しました。']),
  ...
  },
  {
    ...
    sections: [
      ...
      chapter('result', '結果・到達点', '学習を再現可能な測定系まで整備。', ['ランダムエージェントの最大高さ738.6 / 610.6 / 691.5を基準として保存しました。']),
  ...
  },
  {
    ...
    sections: [
      ...
      chapter('result', '結果・到達点', '繰り返し作業を、確認可能な定時処理へ。', ['土日は全日分を保存せず、設定上の標準有効として計算するスパースなモデルにしました。']),
  ...
  },
  {
    ...
    sections: [
      ...
      chapter('result', '結果・到達点', 'カメラ映像を外へ出さず、身体を直接入力に。', ['非対応ブラウザではrequestAnimationFrameへフォールバックします。映像は端末外へ送信しません。']),
  ...
  },
  {
    ...
    sections: [
      ...
      chapter('result', '結果・到達点', '紙のルールを、検証可能な対戦環境へ。', ['C++の探索器をWebAssemblyへ移し、ブラウザ上でも同じ判定を再現しました。']),
  ...
  },
  {
    ...
    sections: [
      ...
      chapter('result', '結果・到達点', '抽選の過程を説明できるデータ構造へ。', ['PENDING / WON_A / WON_B / SKIPPED / LOSTの5状態で、配分後の理由を追えるようにしました。']),
  ...
  },
  {
    ...
    sections: [
      ...
      chapter('result', '結果・到達点', '方向と距離だけで、まとまりとばらつきを両立。', ['30fps・132フレームの比較レンダを約75秒で生成し、時間軸をそろえた検証ができました。']),
