...
await page.waitForTimeout(50)
check(await page.locator('.domain-map li').count() === 3, 'scene 2 did not expose three domain rows')
check(await page.locator('.domain-map li.is-active').count() === 0, 'scene 2 retained an active domain row')
...
await page.locator('.breadth').evaluate((element) => {
  const rect = element.getBoundingClientRect()
  window.scrollTo(0, window.scrollY + rect.top + (rect.height - window.innerHeight) / 2)
})
await page.waitForTimeout(50)
const breadthAtCenter = await page.locator('.breadth-item').evaluateAll((items) => items.map((element) => Number(getComputedStyle(element).getPropertyValue('--item-progress'))))
check(breadthAtCenter.every((progress) => progress === 1), `scene 3 was incomplete at viewport center: ${breadthAtCenter.join(', ')}`)

await page.evaluate(() => window.scrollTo(0, document.documentElement.scrollHeight))
await page.waitForTimeout(50)
const exitAtEnd = await page.locator('.exit-scene').evaluate((element) => Number(getComputedStyle(element).getPropertyValue('--exit-panel-progress')))
check(exitAtEnd === 1, `scene 4 did not complete at document end: ${exitAtEnd}`)
